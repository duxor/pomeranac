...
{{--
    <script>
        ...
        function kreirajNovi(){
            ...
            '</div>'+
            '<div class="form-group">'+
            '<button class="btn btn-lg btn-primary" onclick="Komunikacija.posalji(\'/{{\App\OsnovneMetode::osnovniNav()}}/blog/dodaj-objavu\',\'zaSlanje\',\'poruka\',\'wait\',\'show\')"><i class="glyphicon glyphicon-envelope"></i> PoÅ¡alji</div>'+
            '</div>'+
            '</div>');
            $('#wait').hide();
            $('#show').fadeIn();
        ...
        function getPregled(){
            ...
            $.post('/{{\App\OsnovneMetode::osnovniNav()}}/blog/pregled/{{Session::get('username')}}',{
                ...
                } else $('#show').html('<p>GreÅ¡ka u autentifikaciji. Kontaktirajte administratora.</p>');
                $('#wait').hide();
                $('#show').fadeIn();
            });
        }
